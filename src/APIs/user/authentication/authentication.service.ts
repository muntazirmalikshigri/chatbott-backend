...
//     const phoneInput = normalizedPhoneNumber.startsWith('+')
//         ? normalizedPhoneNumber
//         : `+${normalizedPhoneNumber}`

//     const { countryCode, internationalNumber, isoCode } = parsers.parsePhoneNumber(phoneInput)
...
//     const accessToken = jwt.generateToken({ userId: newUser._id }, config.TOKENS.ACCESS.SECRET, config.TOKENS.ACCESS.EXPIRY)
//     const refreshToken = jwt.generateToken({ userId: newUser._id }, config.TOKENS.REFRESH.SECRET, config.TOKENS.REFRESH.EXPIRY)

//     const tokenDoc: IToken = {
//         token: refreshToken
//     }
//     await tokenRepository.createToken(tokenDoc)
...
export const registrationService = async (payload: IRegisterRequest) => {
    ...
    // EMAIL SERVICE COMMENTED - No email will be sent
    // const confimationURL = `Frontendhost/confimation/${token}?code=${OTP}`
    // const to = [email]
    // const subject = `Confirm your account`
    // const text = `Hey ${name}, Please confirm your account by clicking the link belown\n\n${confimationURL}`
    ...
    logger.info(`User registered: ${email} - Confirmation token: ${token}, OTP: ${OTP}`)
...
export const accountConfirmationService = async (token: string, code: string) => {
    ...
    // EMAIL SERVICE COMMENTED - No welcome email will be sent
    // const to = [user.email]
    // const subject = `Welcome to the base! `
    // const text = `Account has been confirmed.`
    // 
    ...
    logger.info(`Account confirmed for user: ${user.email}`)
